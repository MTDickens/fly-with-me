...
This fork is published from the generated **gh-pages** branch, with **Settings > Pages > Build and deployment > Source: Deploy from a branch**, branch **gh-pages**, folder **/(root)**. Build with `node tools/bundle.mjs`, then publish the contents of `dist/` to that branch. Source work lives on **codex/day-night-seasons**. Keep `.nojekyll`, the worker and data credits beside the generated page.

The optional `.github/workflows/pages.yml` runs the bundler and publishes `dist/` on a push to `main` or a manual workflow run; using it requires changing the Pages source to **GitHub Actions**. Every path is relative, so the existing repository name works at `https://mtdickens.github.io/fly-with-me/`. The ChatGPT Site uses the same `dist/` build and the existing project identified by `.openai/hosting.json` in its source checkout. Do not create another Site for an update.
